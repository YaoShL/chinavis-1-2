...
// 配置参数
const config = {
  excelFilePath: path.join(process.cwd(), 'data', '三维.xlsx'), // Excel文件路径
  outputJsonPath: path.join(process.cwd(), 'src/assets', 'three.json'), // 输出JSON文件路径
...
function convertData(data) {
    return data.map((item, index) => ({
        ...
        metrics: {
            ...
            importance: item['重要性'] || '',
            awareness: item['认知度'] || '',
            influencerank: item['影响力_排名'] || '',
            importancerank: item['重要性_排名'] || '',
            awarenessrank: item['认知度_排名'] || ''
        }
    }));
}
